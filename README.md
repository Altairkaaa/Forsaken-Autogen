...
- the `PY_forsaken_autogen` folder has the full source code (`.py` file) — you can read every single line before running anything
...
## which file/folder do i use?

| option | what it is |
|---|---|
| `forsaken_autogen.exe` | just download and run. easiest option, nothing to install except the driver. recommended for most people |
| `PY_forsaken_autogen/` | raw python source. for tech savvy people, or if the exe gives you issues. requires python + a few pip installs |
...
**1.** download it here: https://github.com/oblitum/Interception/releases

**2.** inside the zip theres a file called `install-interception.exe` — right click it and run as administrator
...
**4.** save all your work then restart your pc (it needs a restart coz its a driver)
